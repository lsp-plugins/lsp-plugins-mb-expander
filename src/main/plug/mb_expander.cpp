...
namespace lsp
{
    namespace plugins
    {
        static constexpr size_t MBE_BUFFER_SIZE     = 0x200;
            ...
            plug::Module(metadata)
        {
            ...
            nEnvBoost       = meta::mb_expander_metadata::FB_DEFAULT;
            bSidechain      = sc;
            bEnvUpdate      = true;
            bUseShmLink     = false;
            bStereoSplit    = false;
        ...
        dspu::sidechain_source_t mb_expander::decode_sidechain_source(int source, bool split, size_t channel)
        {
            if (!split)
            {
                switch (source)
        ...
        dspu::crossover_slope_t mb_expander::classic_xover_slope(size_t slope)
        {
            switch (slope)
            {
                case meta::mb_expander_metadata::SLOPE_6DBO:  return dspu::CROSS_SLOPE_6DBO;
                case meta::mb_expander_metadata::SLOPE_12DBO: return dspu::CROSS_SLOPE_12DBO;
                case meta::mb_expander_metadata::SLOPE_18DBO: return dspu::CROSS_SLOPE_18DBO;
                case meta::mb_expander_metadata::SLOPE_24DBO: return dspu::CROSS_SLOPE_24DBO;
                case meta::mb_expander_metadata::SLOPE_48DBO: return dspu::CROSS_SLOPE_48DBO;
                case meta::mb_expander_metadata::SLOPE_72DBO: return dspu::CROSS_SLOPE_72DBO;
                default: break;
            }
            return dspu::CROSS_SLOPE_48DBO;
        }

        float mb_expander::lp_xover_slope(size_t slope)
        {
            switch (slope)
            {
                case meta::mb_expander_metadata::SLOPE_6DBO:  return -6.0f;
                case meta::mb_expander_metadata::SLOPE_12DBO: return -12.0f;
                case meta::mb_expander_metadata::SLOPE_18DBO: return -18.0f;
                case meta::mb_expander_metadata::SLOPE_24DBO: return -24.0f;
                case meta::mb_expander_metadata::SLOPE_48DBO: return -48.0f;
                case meta::mb_expander_metadata::SLOPE_72DBO: return -72.0f;
                default: break;
            }
            return -48.0f;
        }

        void mb_expander::modern_xover_params(dspu::filter_params_t *fp, modern_filter_t type, size_t slope)
        {
            switch (type)
            {
                case FTYPE_NONE:
                ...
                case FTYPE_LOSHELF:
                    switch (slope)
                    {
                        case meta::mb_expander_metadata::SLOPE_6DBO:
                            fp->nType       = dspu::FLT_BT_RLC_LOSHELF;
                            fp->nSlope      = 1;
                            return;
                        case meta::mb_expander_metadata::SLOPE_12DBO:
                            fp->nType       = dspu::FLT_BT_RLC_LOSHELF;
                            fp->nSlope      = 2;
                            return;
                        case meta::mb_expander_metadata::SLOPE_18DBO:
                            fp->nType       = dspu::FLT_BT_RLC_LOSHELF;
                            fp->nSlope      = 3;
                            return;
                        case meta::mb_expander_metadata::SLOPE_24DBO:
                            fp->nType       = dspu::FLT_BT_LRX_LOSHELF;
                            fp->nSlope      = 1;
                            return;
                        case meta::mb_expander_metadata::SLOPE_48DBO:
                        default:
                            fp->nType       = dspu::FLT_BT_LRX_LOSHELF;
                            fp->nSlope      = 2;
                            return;
                        case meta::mb_expander_metadata::SLOPE_72DBO:
                            fp->nType       = dspu::FLT_BT_LRX_LOSHELF;
                            fp->nSlope      = 4;
                            return;
                    }
        ...
        void mb_expander::sidechain_filter_params(dspu::filter_params_t *fp, modern_filter_t type, size_t slope)
        {
            switch (type)
            {
                case FTYPE_NONE:
        ...
        void mb_expander::init(plug::IWrapper *wrapper, plug::IPort **ports)
        {
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                for (size_t j=0; j<meta::mb_expander_metadata::BANDS_MAX; ++j)
                {
                    ...
                    b->bEnabled     = j < meta::mb_expander_metadata::BANDS_DFL;
                    b->bCustHCF     = false;
                    b->bCustLCF     = false;
                    b->bMute        = false;
                    b->bSolo        = false;
            ...
            float delta = (meta::mb_expander_metadata::CURVE_DB_MAX - meta::mb_expander_metadata::CURVE_DB_MIN) / (meta::mb_expander_metadata::CURVE_MESH_SIZE-1);
            for (size_t i=0; i<meta::mb_expander_metadata::CURVE_MESH_SIZE; ++i)
                vCurve[i]   = dspu::db_to_gain(meta::mb_expander_metadata::CURVE_DB_MIN + delta * i);
        }

        uint32_t mb_expander::decode_sidechain_type(uint32_t sc) const
        {
            if (bSidechain)
            {
                switch (sc)
        ...
        void mb_expander::update_settings()
        {
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                {
                    fp.fFreq        = meta::mb_expander_metadata::FREQ_BOOST_MIN;
                    fp.fFreq2       = 0.0f;
                    fp.fGain        = 1.0f;
                    fp.fQuality     = 0.0f;
                    ...
                    switch (env_boost)
                    {
                        case meta::mb_expander_metadata::FB_BT_3DB:
                            fp.nType        = dspu::FLT_BT_RLC_ENVELOPE;
                            fp.nSlope       = 1;
                            break;
                        case meta::mb_expander_metadata::FB_MT_3DB:
                            fp.nType        = dspu::FLT_MT_RLC_ENVELOPE;
                            fp.nSlope       = 1;
                            break;
                        case meta::mb_expander_metadata::FB_BT_6DB:
                            fp.nType        = dspu::FLT_BT_RLC_ENVELOPE;
                            fp.nSlope       = 2;
                            break;
                        case meta::mb_expander_metadata::FB_MT_6DB:
                            fp.nType        = dspu::FLT_MT_RLC_ENVELOPE;
                            fp.nSlope       = 2;
                            break;
                        case meta::mb_expander_metadata::FB_OFF:
                        default:
                            fp.nType        = dspu::FLT_NONE;
                            fp.nSlope       = 1;
                            break;
            ...
            if (sAnalyzer.needs_reconfiguration())
            {
                ...
                sAnalyzer.get_frequencies(vFreqs, vIndexes, SPEC_FREQ_MIN, SPEC_FREQ_MAX, meta::mb_expander_metadata::MESH_POINTS);
            }
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                for (size_t j=0; j<meta::mb_expander_metadata::BANDS_MAX; ++j)
                {
                    ...
                    bool upward     = b->pMode->value() >= 0.5f;
                    if (enabled && (j > 0))
                    ...
                    b->sSC.set_stereo_mode((nMode == MBEM_MS) ? dspu::SCSM_MIDSIDE : dspu::SCSM_STEREO);
                    ...
                    b->sExp.set_timings(b->pAttTime->value(), b->pRelTime->value());
                    b->sExp.set_hold(b->pHold->value());
                    b->sExp.set_ratio(b->pRatio->value());
                    b->sExp.set_mode((upward) ? dspu::EM_UPWARD : dspu::EM_DOWNWARD);
                    b->sExp.set_knee(b->pKnee->value());
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                if (c->nPlanSize <= 0)
                {
                    ...
                    for (size_t j=0; j<meta::mb_expander_metadata::BANDS_MAX-1; ++j)
                    {
                        exp_band_t * const b        = &c->vBands[j+1];
                    ...
                    for (size_t j=0; j<c->nPlanSize; ++j)
                    {
                        ...
                        b->sEQ[0].freq_chart(b->vSc, vFreqs, meta::mb_expander_metadata::FFT_MESH_POINTS);
                        dsp::pcomplex_mod(b->vSc, b->vSc, meta::mb_expander_metadata::FFT_MESH_POINTS);
        ...
        size_t mb_expander::select_fft_rank(size_t sample_rate)
        {
            const size_t k = (sample_rate + meta::mb_expander_metadata::FFT_XOVER_FREQ_MIN/2) / meta::mb_expander_metadata::FFT_XOVER_FREQ_MIN;
            const size_t n = int_log2(k);
            return meta::mb_expander_metadata::FFT_XOVER_RANK_MIN + n;
        }

        void mb_expander::update_sample_rate(long sr)
        {
            // Determine number of channels
            size_t channels     = (nMode == MBEM_MONO) ? 1 : 2;
            size_t fft_rank     = select_fft_rank(sr);
            size_t bins         = 1 << fft_rank;
            size_t max_delay    = bins + dspu::millis_to_samples(sr, meta::mb_expander_metadata::LOOKAHEAD_MAX);
            ...
            sAnalyzer.init(
                2*channels,
                meta::mb_expander_metadata::FFT_RANK,
                MAX_SAMPLE_RATE,
                meta::mb_expander_metadata::FFT_REFRESH_RATE,
                bins);
            sAnalyzer.set_sample_rate(sr);
            sAnalyzer.set_rank(meta::mb_expander_metadata::FFT_RANK);
            sAnalyzer.set_activity(false);
            sAnalyzer.set_envelope(dspu::envelope::WHITE_NOISE);
            sAnalyzer.set_window(meta::mb_expander_metadata::FFT_WINDOW);
            sAnalyzer.set_rate(meta::mb_expander_metadata::FFT_REFRESH_RATE);
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                {
                    c->sLPXOver.init(fft_rank, meta::mb_expander_metadata::BANDS_MAX);
                    for (size_t j=0; j<meta::mb_expander_metadata::BANDS_MAX; ++j)
        ...
        void mb_expander::ui_deactivated()
        {
            sAnalyzer.set_activity(false);
        }
        ...
        void mb_expander::process_input_mono(float *out, const float *in, size_t count)
        {
            if (in != NULL)
        ...
        void mb_expander::process_input_stereo(float *l_out, float *r_out, const float *l_in, const float *r_in, size_t count)
        {
            if (nMode == MBEM_MS)
            {
                if (l_in != NULL)
        ...
        const float *mb_expander::select_buffer(const exp_band_t *band, const channel_t *channel)
        {
            switch (band->nScType)
            {
        ...
        void mb_expander::preprocess_channel_input(size_t count)
        {
            const size_t channels     = (nMode == MBEM_MONO) ? 1 : 2;
                ...
                for (size_t i=0; i<channels; ++i)
                {
                    ...
                    for (size_t j=0; j<c->nPlanSize; ++j)
                    {
                        ...
                        if (b->bEnabled)
                        {
                            ...
                            if ((enXOver == XOVER_MODERN) && (b->sExp.is_downward()))
                            ...
                            b->pMeterGain->set_value(b->sExp.amplification(lvl));
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                if (sCounter.fired())
                {
                    if (enXOver == XOVER_MODERN)
                    {
                        ...
                        exp_band_t *b       = c->vPlan[0];
                        sFilters.freq_chart(b->nFilterID, c->vTr, vFreqs, b->fGainLevel, meta::mb_expander_metadata::FFT_MESH_POINTS);

                        // Calculate transfer function: other bands
                        for (size_t j=1; j<c->nPlanSize; ++j)
                        {
                            b                   = c->vPlan[j];
                            sFilters.freq_chart(b->nFilterID, vTr, vFreqs, b->fGainLevel, meta::mb_expander_metadata::FFT_MESH_POINTS);
                            dsp::pcomplex_mul2(c->vTr, vTr, meta::mb_expander_metadata::FFT_MESH_POINTS);
                        }
                ...
                for (size_t j=0; j<meta::mb_expander_metadata::BANDS_MAX; ++j)
                {
                    ...
                    if (b->nSync & S_EQ_CURVE)
                    {
                        ...
                        if ((mesh != NULL) && (mesh->isEmpty()))
                        {
                            ...
                            // Fill mesh
                            dsp::copy(&x[2], vFreqs, meta::mb_expander_metadata::MESH_POINTS);
                            dsp::mul_k3(&y[2], b->vSc, b->fScPreamp, meta::mb_expander_metadata::MESH_POINTS);
                            ...
                            y[0]    = 0.0f;
                            y[1]    = y[2];
                            x      += meta::mb_expander_metadata::MESH_POINTS + 2;
                            y      += meta::mb_expander_metadata::MESH_POINTS + 2;
                            ...
                            mesh->data(2, meta::mb_expander_metadata::MESH_POINTS + 4);
                    ...
                    if (b->nSync & S_COMP_CURVE)
                    {
                        ...
                        if ((mesh != NULL) && (mesh->isEmpty()))
                        {
                            if (b->bEnabled)
                            {
                                ...
                                dsp::copy(mesh->pvData[0], vCurve, meta::mb_expander_metadata::CURVE_MESH_SIZE);
                                b->sExp.curve(mesh->pvData[1], vCurve, meta::mb_expander_metadata::CURVE_MESH_SIZE);
                                if (b->fMakeup != GAIN_AMP_0_DB)
                                    dsp::mul_k2(mesh->pvData[1], b->fMakeup, meta::mb_expander_metadata::CURVE_MESH_SIZE);

                                // Mark mesh containing data
                                mesh->data(2, meta::mb_expander_metadata::CURVE_MESH_SIZE);
                            }
                            else
                                mesh->data(2, 0);
                ...
                if ((mesh != NULL) && (mesh->isEmpty()))
                {
                    if (c->bInFft)
                    {
                        ...
                        mesh->pvData[0][meta::mb_expander_metadata::FFT_MESH_POINTS+1] = SPEC_FREQ_MAX * 2.0f;
                        mesh->pvData[1][0] = 0.0f;
                        mesh->pvData[1][meta::mb_expander_metadata::FFT_MESH_POINTS+1] = 0.0f;

                        // Copy frequency points
                        dsp::copy(&mesh->pvData[0][1], vFreqs, meta::mb_expander_metadata::FFT_MESH_POINTS);
                        sAnalyzer.get_spectrum(c->nAnInChannel, &mesh->pvData[1][1], vIndexes, meta::mb_expander_metadata::FFT_MESH_POINTS);

                        // Mark mesh containing data
                        mesh->data(2, meta::mb_expander_metadata::FFT_MESH_POINTS + 2);
                    }
                    else
                        mesh->data(2, 0);
                }
                ...
                if ((mesh != NULL) && (mesh->isEmpty()))
                {
                    ...
                    {
                        // Copy frequency points
                        dsp::copy(mesh->pvData[0], vFreqs, meta::mb_expander_metadata::FFT_MESH_POINTS);
                        sAnalyzer.get_spectrum(c->nAnOutChannel, mesh->pvData[1], vIndexes, meta::mb_expander_metadata::FFT_MESH_POINTS);

                        // Mark mesh containing data
                        mesh->data(2, meta::mb_expander_metadata::FFT_MESH_POINTS);
                    }
                    else
                        mesh->data(2, 0);
                }
                ...
                if ((mesh != NULL) && (mesh->isEmpty()))
                {
                    ...
                    dsp::copy(mesh->pvData[0], vFreqs, meta::mb_expander_metadata::FFT_MESH_POINTS);
                    dsp::copy(mesh->pvData[1], c->vTrMem, meta::mb_expander_metadata::FFT_MESH_POINTS);
                    mesh->data(2, meta::mb_expander_metadata::FFT_MESH_POINTS);
                }
            } // for channel
        ...
        bool mb_expander::inline_display(plug::ICanvas *cv, size_t width, size_t height)
        {
            // Check proportions
            ...
            size_t channels     = ((nMode == MBEM_MONO) || ((nMode == MBEM_STEREO) && (!bStereoSplit))) ? 1 : 2;
            const uint32_t *vc  = (channels == 1) ? &c_colors[0] :
                                  (nMode == MBEM_MS) ? &c_colors[3] :
                                  &c_colors[1];
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                {
                    size_t k        = (j*meta::mb_expander_metadata::MESH_POINTS)/width;
                    b->v[0][j+1]    = vFreqs[k];
        ...
        void mb_expander::dump(dspu::IStateDumper *v) const
        {
            plug::Module::dump(v);
            ...
            v->begin_array("vChannels", vChannels, channels);
            {
                for (size_t i=0; i<channels; ++i)
                {
                    ...
                    for (size_t i=0; i<meta::mb_expander_metadata::BANDS_MAX; ++i)
                    {
                        const exp_band_t * const b = &c->vBands[i];
                        v->begin_object(b, sizeof(exp_band_t));
                        {
                    ...
                    v->begin_array("vSplit", c->vBands, meta::mb_expander_metadata::BANDS_MAX-1);
                    for (size_t i=0; i<(meta::mb_expander_metadata::BANDS_MAX-1); ++i)
                    {
                        const split_t * const s = &c->vSplit[i];
                    ...
                    v->end_array();
                    v->writev("vPlan", c->vPlan, meta::mb_expander_metadata::BANDS_MAX);
