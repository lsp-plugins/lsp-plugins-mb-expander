...
namespace lsp
{
    namespace plugins
    {
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                if (c->nPlanSize <= 0)
                {
                    ...
                    for (size_t j=0; j<c->nPlanSize; ++j)
                    {
                        ...
                        b->sEQ[0].freq_chart(size_t(0), b->vTr, vFreqs, meta::mb_expander_metadata::FFT_MESH_POINTS);
                        b->sEQ[0].freq_chart(size_t(1), vTr, vFreqs, meta::mb_expander_metadata::FFT_MESH_POINTS);
                        dsp::pcomplex_mul2(b->vTr, vTr, meta::mb_expander_metadata::FFT_MESH_POINTS);
        ...
        void mb_expander::update_sample_rate(long sr)
        {
            // Determine number of channels
            size_t channels     = (nMode == MBEM_MONO) ? 1 : 2;
            size_t max_delay    = dspu::millis_to_samples(sr, meta::mb_expander_metadata::LOOKAHEAD_MAX);
